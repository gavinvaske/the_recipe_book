...
export const getMaterialLengthAdjustments = async (searchQuery: SearchQuery): Promise<SearchResult<any>> => {
  ...
  const response : AxiosResponse<SearchResult<IMaterialLengthAdjustment>> = await axios.get(
    `/material-length-adjustments/search?query=${query || ''}&pageIndex=${pageIndex}&limit=${pageSize}` 
